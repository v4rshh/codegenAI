...
def generate_ollama_response(chat_id, model="phi3", extra_context=""):
    messages = [
        {
            ...
            "content": (
                "You are CODEGEN AI. "
                "Whenever you output code, ALWAYS wrap it in Markdown "
                "code blocks using triple backticks with the correct language."
            )
        }
    ]
    ...
    if extra_context:
        messages.append({
            "role": "system",
            "content": f"Use the following files as context:\n{extra_context}"
        })
...
with st.chat_message("assistant", avatar="🤖"):
    ...
    full_response = ""
    # Render response exactly as markdown (keeps ```python blocks)
    message_placeholder.markdown(assistant_response, unsafe_allow_html=False)
